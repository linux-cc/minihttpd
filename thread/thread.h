...
public:
    ...
    void cancel() { pthread_cancel(_pid); }
    void join() { pthread_join(_pid, NULL); }
    bool isAlive() { return pthread_kill(_pid, 0) == 0; }
    void testCancel() { pthread_testcancel(); }
    pthread_t pid() const { return _pid; }
    Runnable *runnable() const { return _run; }
...
class AutoMutex {
public:
    AutoMutex(Mutex &metux):_mutex(metux), _locked(false) { _locked = _mutex.lock(); }
    ~AutoMutex() { if (_locked) _mutex.unlock(); }
private:
    Mutex &_mutex;
    bool _locked;
};
